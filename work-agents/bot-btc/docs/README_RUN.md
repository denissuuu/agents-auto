...
```bash
PYTHONPATH=src python3 src/backtest.py --data data/btc_history.csv --json
PYTHONPATH=src python3 src/backtest.py --walk-forward --folds 3 --json
```

Si le fichier local manque, le programme propose `--public-data`, qui lit uniquement les klines publiques BTCEUR. Cette option ne peut passer aucun ordre et ne lit aucune clé.
