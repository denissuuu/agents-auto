# Paramètres de la stratégie BTC (hypothèses de test)
...
- Le prix et la comptabilité sont en EUR (`BTCEUR`, klines publiques Binance) ; la validité d'un résultat reste bornée par la qualité des données et les hypothèses de frais/liquidité.
- `strategy_mode` (défaut `buy_hold`) peut passer à `sma_rsi` ou `sma_only`. `buy_hold` : une seule entrée en tendance haussière exposant ~95 % du capital (pas d'achats répétés), la seule vente est le stop-loss. Conséquence : drawdown potentiellement élevé (72 % sur 999 jours BTC/EUR), à comparer au buy-and-hold pur (+137,7 % vs +61,8 % net).
