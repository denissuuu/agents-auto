"""Backtest BTC sans look-ahead, avec coûts et registres paper.
...
DEFAULT_DATA = os.path.join(ROOT, "data", "btc_history.csv")
...
def fetch_binance_candles(
    ...
    if not str(symbol).upper().startswith("BTC"):
        raise ValueError("le backtest public ne doit concerner que BTC")
    ...
    if interval_ms:
        ...
        unique = closed
    if not unique:
        raise ValueError("aucune kline publique BTC reçue")
    return unique, warnings


def fetch_closes(symbol: str = "BTCEUR", interval: str = "1h", limit: int = 100):
...
def _process_intrabar_exits(portfolio: Portfolio, candle: Candle, exit_policy: str = "full") -> None:
    ...
    if portfolio.stop_loss_pct > 0 and (candle.open <= stop or candle.low <= stop):
        ...
        _execute_exit(portfolio, ExitInstruction(
            "stop_loss", portfolio.btc, stop, None
    ...
    if state.get("trailing_active") and portfolio.trailing_stop_pct > 0:
        ...
        if candle.open <= trigger or candle.low <= trigger:
            _execute_exit(portfolio, ExitInstruction(
                "trailing_stop", portfolio.btc, trigger, None
    ...
    if opened and portfolio.max_position_days > 0:
        try:
            ...
            if (current_time - opened_time).total_seconds() / 86400 >= portfolio.max_position_days:
                _execute_exit(portfolio, ExitInstruction(
                    "max_duration", portfolio.btc, candle.close, None
    ...
    while portfolio.btc > EPS:
        ...
        if candle.high < target:
            break
        quantity = portfolio.btc * portfolio.profit_fractions[next_level]
        if quantity <= EPS:
...
def run_backtest(
    ...
    for index in range(start_index, end_index):
        ...
        if pending_signal == "BUY":
            ...
            if strategy_mode == "buy_hold" and portfolio.btc > EPS:
                pass
            else:
    ...
    if quote and not str(cfg.get("symbol", "BTCEUR")).upper().endswith(quote):
        all_warnings.append(
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Backtest BTC paper-only, sans ordre réel")
