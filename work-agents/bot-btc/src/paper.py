"""Paper trading BTC : état local et prix publics, aucun ordre réel.
...
def get_context(config: Dict[str, Any]) -> List[float]:
    ...
    symbol = str(config.get("symbol", "BTCEUR")).upper()
    ...
    if not closes:
        raise RuntimeError("historique public BTC vide")
    return closes
...
def step(config: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    if signal == "SELL" and mode != "buy_hold":
        ...
        if signal == "BUY" and not reasons and not (mode == "buy_hold" and portfolio.btc > EPS):
    ...
    line = (
        ...
        f"reserve_fees={snapshot['fee_reserve_cash']:.8f} btc={snapshot['btc']:.10f} "
    ...
    if executed:
        block = (
            f"[{now}] paper BTC signal={signal} prix={current_price:.8f} "
        ...
        with open(TRADES, "a", encoding="utf-8") as handle:
            handle.write(block)
        try:
            notify.alert("Trade BTC simulé", f"{','.join(reasons)} — aucun ordre réel")
        except Exception:
            pass
    print(line.strip())
    return snapshot
