...
def main():
    ...
    try:
        ...
        live = float(price_mod.get_price(str(config.get("symbol", "XRPEUR")).upper()))
    except Exception:
...
XRP simulé : {xrp:.6f} — P&amp;L réalisé : {realized:.2f} — P&amp;L non réalisé : {unrealized:.2f} — frais : {fees:.2f}<br>
