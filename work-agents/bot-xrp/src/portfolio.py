"""Registre et moteur d'exécution paper-only pour XRP spot.

Ce module ne contient aucun client d'ordres. Il conserve le cash, la quantité
XRP, les frais, les P&L et les réservations ; chaque achat est vérifié avant
mutation de l'état.
"""
...
class Portfolio:
    """Compte paper XRP avec comptabilité FIFO et contrôle de solvabilité."""
    ...
    def __init__(
        ...
        if not self.symbol.startswith("XRP"):
            raise ValueError("Seul XRP spot est autorisé")
        if not self.paper:
    ...
    def _assert_invariants(self) -> None:
        ...
        if self.xrp < -EPS or any(lot.quantity < -EPS for lot in self.positions):
            raise TradingError("XRP ne peut pas être négatif")
        ...
        for lot in self.positions:
            if lot.cost_basis < -EPS or lot.entry_price <= 0:
                raise TradingError("lot XRP incohérent")
    ...
    def _consume_cost_basis(self, quantity: float) -> float:
        ...
        if remaining > EPS:
            raise TradingError("vente supérieure à la quantité XRP détenue")
        return removed
    ...
    def plan_exits(
        ...
        if self.stop_loss_pct > 0 and mark <= stop_price:
            return [ExitInstruction("stop_loss", self.xrp, stop_price)]
        if self.max_position_days > 0 and self._age_days(now) >= self.max_position_days:
            return [ExitInstruction("max_duration", self.xrp, mark)]
        ...
        if self.position_state.get("trailing_active") and self.trailing_stop_pct > 0:
            ...
            if mark <= trailing_price:
                return [ExitInstruction("trailing_stop", self.xrp, trailing_price)]
        ...
        while next_level < len(self.profit_levels):
            ...
            qty = min(self.xrp, self.xrp * fraction)
            if qty > EPS:
    ...
    def from_dict(
        ...
        **kwargs: Any,
        ...
        """Recharge un registre, avec migration conservative des anciens XRP."""
        ...
        if isinstance(raw_positions, list) and raw_positions:
            ...
            legacy_qty = max(0.0, _number(data.get("xrp", 0)))
            if legacy_qty > EPS:
...
def portfolio_from_config(config: Dict[str, Any]) -> Portfolio:
    ...
    return Portfolio(
        ...
        symbol=str(config.get("symbol", "XRPEUR")),
